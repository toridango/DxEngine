...
VolumetricBalloonShader::VolumetricBalloonShader(HWND hwnd, ID3D11Device* device, ID3D11DeviceContext* immediateContext) :
...
VolumetricBalloonShader::~VolumetricBalloonShader()
{
	if (m_cameraBuffer)
	{
...
bool VolumetricBalloonShader::InitializeShader(ShaderFiles sf)
{
	return InitializeShader(sf.vs, sf.ps);
}

bool VolumetricBalloonShader::InitializeShader(WCHAR* vsFilename, WCHAR* psFilename)
{
	...
	m_psFunctionName = "VolumetricBalloonPixelShader";
	m_vsFunctionName = "VolumetricBalloonVertexShader";
...
bool VolumetricBalloonShader::SetShaderParameters(XMMATRIX worldMatrix,
	XMMATRIX viewMatrix, XMMATRIX projectionMatrix, XMFLOAT3 cameraPosition,
	XMFLOAT3 cPos, float deltavalue)
{
	HRESULT result;
...
bool VolumetricBalloonShader::Render(GameObject* go, CameraClass* camera, float deltavalue)
{
	XMFLOAT3 cPos;
