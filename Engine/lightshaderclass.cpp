							...
							XMFLOAT3 cameraPosition, XMFLOAT4 specularColour, float specularPower, float deltavalue, ID3D11ShaderResourceView* texture)
{
	...
	result = SetShaderParameters(deviceContext, worldMatrix, viewMatrix, projectionMatrix, lightDirection, ambientColour, diffuseColour, cameraPosition, specularColour, specularPower, deltavalue, texture);
	if(!result)
	{
		return false;
	}
